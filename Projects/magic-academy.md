...
- 컴포넌트 간 인터페이스 계약(Agent Runtime ↔ Tick Engine ↔ Magic Layer)이 중반에 바뀌면서 조율 비용이 컸음 → 인터페이스 스펙을 Slice 시작 전에 먼저 확정하고 변경 시 전파 범위를 명시
- LLM 호출 비용·latency를 실제로 측정한 적 없음 → 다음엔 Agent당 평균 응답 시간·비용을 초기부터 기록해 병목 파악
- Railway 배포 후 실제 시뮬레이션 품질을 평가하지 않음 → 배포 직후 시나리오 기반 품질 체크리스트를 두고 검증

## 아쉬운 것

- 팀원 동기부여를 더 적극적으로 했어야 했음 — 후반으로 갈수록 참여 에너지가 떨어지는 게 느껴졌는데 대응이 늦었음
- 컴포넌트 간 인터페이스 계약(Agent Runtime ↔ Tick Engine ↔ Magic Layer)이 중반에 바뀌면서 조율 비용이 컸음 → 인터페이스 스펙을 Slice 시작 전에 먼저 확정하고 변경 시 전파 범위를 명시
- LLM 호출 비용·latency를 실제로 측정한 적 없음 → 다음엔 Agent당 평균 응답 시간·비용을 초기부터 기록해 병목 파악
- Railway 배포 후 실제 시뮬레이션 품질을 평가하지 않음 → 배포 직후 시나리오 기반 품질 체크리스트를 두고 검증
